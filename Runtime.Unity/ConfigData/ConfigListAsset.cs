...
{
    public abstract class ConfigListAssetPtr : ScriptableObject, IConfigListPtr, IValidatable
    {
        [TextArea(0, 3)]
    ...
    public abstract class ConfigListAsset<T> : ConfigListAssetPtr, IConfigList<T> where T : IConfig
    {
        public T[] Configs
        {
            get
...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        public IReadOnlyList<T> GetConfigs() => Configs;
        ...
        {
            if (_table == null /*|| _configTable.Count != Configs.Length*/)
            {
            ...
            foreach (T config in Configs)
            {
                ...
                {
                    Debugger.LogError($"Duplicate config id: {config.Id}. Config type: {typeof(T)}.", this, GetLogTag());
                }
            }
        ...
        {
            return $"GBG|ConfigListAsset<{GetConfigType().Name}>";
        }
    }
}
